...
class PublicController extends \Ip\Controller
{
    ...
    protected function systemCheck()
    {
        ...
        // Register website
        Helper::registerWebsite('systemCheck', $notSuccess);
    ...
    protected function finish()
    {
        ...
        $data = array(
            'showInfo' => $showInfo,
            'autoGeneratedPassword' => isset($_SESSION['autoGeneratedPassword']) ? $_SESSION['autoGeneratedPassword'] : null
        );
    ...
    public function testConfiguration()
    {
        ...
        // Optional
        if (ipRequest()->getPost('configExpanded') == 1) {

            // Admin username
            // ipRequest()->getPost('configAdminUsername')

            // Admin password
            // ipRequest()->getPost('configAdminPassword')

            // Admin email
            if (!Helper::validateAdminEmail(ipRequest()->getPost('configAdminEmail'))) {
                $errors[] = __('Please enter correct administrator email.', 'Install', false);
            }
        }
        ...
        $_SESSION['config']['expanded'] = ipRequest()->getPost('configExpanded');
        $_SESSION['config']['adminUsername'] = ipRequest()->getPost('configAdminUsername');
        $_SESSION['config']['adminPassword'] = ipRequest()->getPost('configAdminPassword');
        $_SESSION['config']['adminEmail'] = ipRequest()->getPost('configAdminEmail');
        $_SESSION['config']['developmentEnvironment'] = ipRequest()->getPost('configDevelopmentEnvironment');
        $_SESSION['config']['showErrors'] = ipRequest()->getPost('configShowErrors');
        $_SESSION['config']['debugMode'] = ipRequest()->getPost('configDebugMode');

        // Register website
        Helper::registerWebsite('configuration', $errors);
    ...
    public function createDatabase()
    {
        ...
        $configToFile['developmentEnvironment'] = $_SESSION['config']['developmentEnvironment'];
        $configToFile['showErrors'] = $_SESSION['config']['showErrors'];
        $configToFile['debugMode'] = $_SESSION['config']['debugMode'];

        if (Helper::checkModRewrite() != 'success') {
            $configToFile['rewritesDisabled'] = true;
        }

        $adminUsername = $_SESSION['config']['adminUsername'];
        $adminEmail = $_SESSION['config']['adminEmail'];
        $adminPassword = $_SESSION['config']['adminPassword'];
        $_SESSION['autoGeneratedPassword'] = null; // resetting

        // if administrator username is not given, website email will be used
        if (empty($adminUsername)) {
            $adminUsername = $_SESSION['config']['websiteEmail'];
        }
        // if administrator email is not given, website email will be used
        if (empty($adminEmail)) {
            $adminEmail = $_SESSION['config']['websiteEmail'];
        }
        // if administrator password is not given, it will be randomly generated and presented to the user
        if (empty($adminPassword)) {
            $adminPassword = Helper::randString(8);
            $_SESSION['autoGeneratedPassword'] = $adminPassword;
        }
        ...
        // Register website
        Helper::registerWebsite('database');

        return \Ip\Response\JsonRpc::result(array('redirect' => 'index.php?step=4'));
    }
}
