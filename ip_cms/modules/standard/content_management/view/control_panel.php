<div id="ipControllPanel">
    ...
    <?php } else { ?>
        <div style="float: left;">
            <p>This is a preview of older revision, created at (<?php echo date("Y-m-d H:i", $currentRevision['created']) ?>).</p>
            <p><a>Publish this revision</a></p>
            <p><a>Duplicate and edit this revision</a></p>
        </div>
        ...
        <div>
            <select id="ipRevisionSelect">
                <?php foreach ($revisions as $revisionKey => $revision){ ?>
                    <option <?php echo $revision['revisionId'] == $currentRevision['revisionId'] ? ' selected="selected" ' : '' ?> value="<?php echo $managementUrls[$revisionKey]; ?>"><?php echo (int)$revision['revisionId'].' - '.date("Y-m-d H:i", $revision['created']); echo $revision['published'] ? ' (published) ' : ''; ?></option>
                <?php } ?>
            </select>
        </div>
        <div>
