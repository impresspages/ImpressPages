...
function content_mod_file() {
	...
	function preview(worker_form, return_script, collection_number) {
		...
		document.getElementById(worker_form).innerHTML = ''
				...
				+ collection_number
				+ '" />'
				+ '<input name="module_key" value="file" />'
	...
	function manage() {
		...
		if (this.new_photo != '')
			current_photo_img = '' + this.new_photo + '';
		else {
			if (this.link_to_existing_photo != '')
				current_photo_img = '' + this.link_to_existing_photo + '';
			else
				current_photo_img = '';
		}
		...
		div.innerHTML = ''
				...
				+ '" target="mod_file_iframe" method="post" enctype="multipart/form-data">'
				...
				+ '</label>'
				+ '<div class="ipCmsClear"></div>'
				+ '<input type="hidden" name="action" value="upload_tmp_file"/>'
				+ '</form>' + '' + '' + '' + '' + '</div>';
		return div;
	}
	;
	...
	function save(forced) {
		...
		if (document
			...
			document.getElementById('mod_file_action_after_photo_save').value = this.my_name
	...
	function response_after_photo_save(forced) {
		var iFrameDocObj = window.frames['mod_file_iframe'].window.document;
		if (iFrameDocObj.body.innerHTML != '') {
			var name0 = '';

			errors = window.frames['mod_file_iframe'].errors;

			if (errors.length == 0 || errors[0] == 4 || forced) {
				if (errors.length == 0)
					this.new_photo = window.frames['mod_file_iframe'].variables[0];
				this.menu_management
						.module_preview_save_response(this.collection_number);
			} else {
				var answer = '';
				...
				case '3':
					...
					break;
				case '4':
					answer = widget_file_no_file;
					break;
				case '6':
				case '7':
					answer = widget_file_unknown;
					break;
				case '8':
					answer = widget_file_bad_type;
					break;
				}
