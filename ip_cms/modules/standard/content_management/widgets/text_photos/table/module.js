...
function content_mod_table() {
  ...
  function preview(worker_form, return_script, collection_number) {
    ...
    + '" />' + '<input name="module_key" value="table" />'
  ...
  function manage() {
    ...
    + widget_table_widget_title
    + '</label>'
    + '<form id="mod_' + this.collection_number	+ '_layout" action="">'	+ mod_table_layout + '</form>'
    + '<div class="ipCmsModuleSeparator"></div>'
    + '<textarea style="width: 100%;" id="management_' + this.collection_number + '_table"></textarea>'
    + '</div>';
    //		div.firstChild.value = this.text;
  ...
  function manage_init() {
    ...
    eval(configWidgetTextPhotosTableMceInit);

    tinyMCE.execCommand('mceAddControl', true,
      'management_' + this.collection_number + '_table');
  }
  ...
  function save() {
    ...
    tinyMCE.execCommand('mceFocus', false, 'management_' + this.collection_number + '_table');
    this.text = tinyMCE.get('management_' + this.collection_number + '_table').getContent();// or tinyMCE.activeEditor.getContent()
