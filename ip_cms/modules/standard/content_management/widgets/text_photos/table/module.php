...
class Module extends \Modules\standard\content_management\Widget{
  ...
  function init(){
    ...
    global $site;    
    $answer = '';
    $answer .= ' <script type="text/javascript"  src="'.BASE_URL.CONTENT_MODULE_URL.'text_photos/table/module.js"></script>';
  ...
  function getLayout($id){
    $sql = "select * from `".DB_PREF."mc_text_photos_table` where id = '".(int)$id."'";
    $rs = mysql_query($sql);
    if($rs){
  ...
  function add_to_modules($mod_management_name, $collection_number, $module_id, $visible){ //add existing module from database to javascript array
    ...
    if (!$rs || !$lock = mysql_fetch_assoc($rs))
      ...
      $answer .= '<script type="text/javascript">
                  //<![CDATA[
                  ';
      $answer .= "  var new_module = new content_mod_table();";
  ...
  function create_new_instance($values){
    $sql = "insert into `".DB_PREF."mc_text_photos_table` set layout= '".mysql_real_escape_string($values['layout'])."' , text = '".mysql_real_escape_string($values['text'])."' ";
    $rs = mysql_query($sql);
    if(!$rs){
      return "Can't insert new module. ".$sql;
    }else{
      $sql = "select max(id) as max_id from `".DB_PREF."mc_text_photos_table` where 1";
      $rs = mysql_query($sql);
      if (!$rs)
  ...
  function update($values){
    ...
    return("Can't update module row number".$sql);
      ...
      $sql = "update `".DB_PREF."mc_text_photos_table` set layout = '".mysql_real_escape_string($values['layout'])."', `text` = '".mysql_real_escape_string($values['text'])."' where id = '".(int)$values['id']."'  ";
      if (!mysql_query($sql))
  ...
  function delete($values){
    ...
    return("Can't delete element to module association ".$sql);
      ...
      $sql = "delete from `".DB_PREF."mc_text_photos_table` where id = '".(int)$values['id']."' ";
      if (!mysql_query($sql))
  ...
  function delete_by_id($id){
    ...
    trigger_error("Can't delete element to module association ".$sql);
      ...
      $sql = "delete from `".DB_PREF."mc_text_photos_table` where id = '".(int)$id."' ";
      if (!mysql_query($sql))
  ...
  function make_html($id){
    ...
    $site->requireTemplate('standard/content_management/widgets/'.GROUP_KEY.'/'.MODULE_KEY.'/template.php');
    $sql = "select text from `".DB_PREF."mc_text_photos_table` where id = '".(int)$id."' ";
    $rs = mysql_query($sql);
    if ($rs){
      if ($lock = mysql_fetch_assoc($rs)){
  ...
  function clearCache($cachedBaseUrl) {
    $sql = "update `".DB_PREF."mc_text_photos_table` set `text` = REPLACE(`text`, '".mysql_real_escape_string($cachedBaseUrl)."', '".mysql_real_escape_string(BASE_URL)."')  where 1 ";
    $rs = mysql_query($sql);
    if (!$rs) {
      trigger_error($sql." ".mysql_error());
    }
  ...
  function updateLinks($oldUrl, $newUrl) {
    $sql = "update `".DB_PREF."mc_text_photos_table` set `text` = REPLACE(`text`, '".mysql_real_escape_string($oldUrl)."', '".mysql_real_escape_string($newUrl)."') where 1 ";
    $rs = mysql_query($sql);
    if (!$rs) {
      trigger_error($sql." ".mysql_error());
    }
