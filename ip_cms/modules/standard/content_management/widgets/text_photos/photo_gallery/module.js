 ...
 */
function content_mod_photo_gallery() {
	...
	function preview(worker_form, return_script, collection_number) {
		document.getElementById(worker_form).innerHTML = ''
				...
				+ '" />' + '<input name="module_key" value="photo_gallery" />'
		...
		for ( var i = 0; i < this.photos.length; i++) {
			...
			if (this.photos[i].new_photo != '') {
				...
				bigphoto = global_config_tmp_image_url
	...
	function manage() {
		...
		div.innerHTML = ''
				...
				+ '" target="mod_photo_gallery_iframe" method="post" enctype="multipart/form-data">'
				+ '<label class="ipCmsTitle">'
				+ widget_photo_gallery_new_photo
				+ '</label>'
				...
				+ widget_photo_gallery_width
				+ '"/>'
				+ '<input type="hidden" name="photo_height[]" value="'
				+ widget_photo_gallery_height
				+ '"/>'
				...
				+ '"/>'
				+ '<input type="hidden" name="photo_height[]" value="'
				+ widget_photo_gallery_big_height
				+ '"/>'
				+ '<input type="hidden" name="photo_quality[]" value="'
				...
				+ '<div id="photo_gallery_' + this.collection_number
	...
	function redraw_gallery_management() {
		var div2 = document
				.getElementById('photo_gallery_' + this.collection_number + '_photos');
		if (div2)
	...
	function upload_new_photo() {
		...
		this.new_photo = new gallery_photo();
		this.new_photo.init();
		this.new_photo.title = document
				.getElementById('management_' + this.collection_number + '_title').value;
		document.getElementById('mod_photo_gallery_action_after_photo_save').value = this.my_name + '.response_after_photo_save()';
		var form = document
	...
	function response_after_photo_save() {
		var iFrameDocObj = window.frames['mod_photo_gallery_iframe'].window.document;
		...
		if (iFrameDocObj.body.innerHTML != '') {
			...
			var errors;
			/*
			 * try{ eval(iFrameDocObj.body.innerHTML); }catch(e){
			 * alert("Unexpected error " + iFrameDocObj.body.innerHTML); }
			 */
			name0 = window.frames['mod_photo_gallery_iframe'].variables[0];
			name1 = window.frames['mod_photo_gallery_iframe'].variables[1];
			errors = window.frames['mod_photo_gallery_iframe'].errors;
			if (errors.length == 0) {
				this.new_photo.set_new_photo(name0);
				this.new_photo.set_new_bigphoto(name1);
				...
				case '3':
					...
					break;
				case '4':
					answer = widget_photo_gallery_no_file;
					break;
				case '6':
				case '7':
					answer = widget_photo_gallery_unknown;
					break;
				case '8':
					answer = widget_photo_gallery_bad_type;
					break;
				}
	...
	function store_to_db_fields(row_number, menu_element_id) {
		if (this.id == null) {
			if (this.deleted == 0) {
				...
				fields.push( [ 'module_key', 'photo_gallery' ]);
