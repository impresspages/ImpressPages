...
class Module extends \Modules\standard\content_management\Widget{
  ...
  function add_to_modules($mod_management_name, $collection_number, $module_id, $visible){ //add existing module from database to javascript array
    ...
    $sql = "select text from ".DB_PREF."mc_text_photos_text where id = '".(int)$module_id."' ";
    $rs = mysql_query($sql);
    ...
    trigger_error("Can't get module information ".$sql);
      ...
      $answer .= '<script type="text/javascript">
                  //<![CDATA[
                  ';
      $answer .= "  var new_module = new content_mod_text();";
      ...
      $answer .= "  new_module.preview_html = '".str_replace("\r", " ",str_replace("\n", " ",str_replace("'", "\\'", str_replace("\\", "\\\\",Template::generateHtml($lock['text'], $this->getLayout($module_id))))))."';";
  ...
  function create_new_instance($values){
    $sql = "insert into ".DB_PREF."mc_text_photos_text set layout= '".mysql_real_escape_string($values['layout'])."' , text = '".mysql_real_escape_string($values['text'])."' ";
    $rs = mysql_query($sql);
    if(!$rs){
      return "Can't insert new module. ".$sql;
    }else{
      $sql = "select max(id) as max_id from ".DB_PREF."mc_text_photos_text where 1";
      $rs = mysql_query($sql);
      if (!$rs)
  ...
  function delete($values){
    ...
    return("Can't delete element to module association ".$sql);
      ...
      $sql = "delete from ".DB_PREF."mc_text_photos_text where id = '".(int)$values['id']."' ";
      if (!mysql_query($sql))
  ...
  function delete_by_id($id){
    ...
    trigger_error("Can't delete element to module association ".$sql);
      ...
      $sql = "delete from ".DB_PREF."mc_text_photos_text where id = '".(int)$id."' ";
      if (!mysql_query($sql))
  ...
  function make_html($id){
    ...
    $site->requireTemplate('standard/content_management/widgets/'.GROUP_KEY.'/'.MODULE_KEY.'/template.php');
    $sql = "select text from ".DB_PREF."mc_text_photos_text where id = '".(int)$id."' ";
    $rs = mysql_query($sql);
    if ($rs){
      if ($lock = mysql_fetch_assoc($rs)){
