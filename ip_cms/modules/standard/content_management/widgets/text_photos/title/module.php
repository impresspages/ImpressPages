...
class Module extends \Modules\standard\content_management\Widget{
  ...
  function init(){
    global $site;    
    $answer =  '<script type="text/javascript"  src="'.BASE_URL.CONTENT_MODULE_URL.'text_photos/title/module.js"></script>';
    $answer .=  '
		<script type="text/javascript">
		  //<![CDATA[
			function menu_mod_title_select_level(collection_number, level){
				i = 1;
				while(document.getElementById(\'management\' + collection_number + \'_title_level_\' + i)){
					document.getElementById(\'management\' + collection_number + \'_title_level_\' + i).src = \''.BASE_URL.MODULE_DIR.'standard/content_management/widgets/text_photos/title/design/mod_title_h\' + i + \'.gif\';
					i++;
				}
					
				document.getElementById(\'management_\' + collection_number + \'_level\').value = level;
				document.getElementById(\'management\' + collection_number + \'_title_level_\' + level).src=\''.BASE_URL.MODULE_DIR.'standard/content_management/widgets/text_photos/title/design/mod_title_h\' + level + \'_act.gif\'; 
  ...
  function getLayout($id){
    $sql = "select * from `".DB_PREF."mc_text_photos_title` where id = '".(int)$id."'";
    $rs = mysql_query($sql);
    if($rs){
  ...
  function add_to_modules($mod_management_name, $collection_number, $module_id, $visible){ //add existing module from database to javascript array
    ...
    $sql = "select title, level from `".DB_PREF."mc_text_photos_title` where id = '".(int)$module_id."' ";
    $rs = mysql_query($sql);
    ...
    trigger_error("Can't get module information ".$sql);
      ...
      $answer .= '<script type="text/javascript">
                  //<![CDATA[
                  ';
      $answer .= "  var new_module = new content_mod_title();";
      ...
      $answer .= "  new_module.preview_html = '".str_replace("'", "\\'", Template::generateHtml($lock['title'], $lock['level'], $this->getLayout($module_id)))."';";
      ...
      $answer .= "  new_module.set_level(".addslashes($lock['level']).");";
  ...
  function create_new_instance($values){
    $sql = "insert into `".DB_PREF."mc_text_photos_title` set layout= '".mysql_real_escape_string($values['layout'])."' , title = '".mysql_real_escape_string($values['title'])."' , level = '".mysql_real_escape_string($values['level'])."'";
    $rs = mysql_query($sql);
    if(!$rs){
      return "Can't insert new module. ".$sql;
    }else{
      $sql = "select max(id) as max_id from `".DB_PREF."mc_text_photos_title` where 1";
      $rs = mysql_query($sql);
      if (!$rs)
      return "Can't get last inserted id ".$sql;
        ...
        $sql = "insert into `".DB_PREF."content_element_to_modules` set".
            ...
            ", group_key='text_photos', module_key='title', module_id = '".(int)$lock['max_id']."'".
  ...
  function delete($values){
    ...
    $this->set_error("Can't delete element to module association ".$sql);
      ...
      $sql = "delete from `".DB_PREF."mc_text_photos_title` where id = '".(int)$values['id']."' ";
      if (!mysql_query($sql))
  ...
  function delete_by_id($id){
    ...
    trigger_error("Can't delete element to module association ".$sql);
      ...
      $sql = "delete from `".DB_PREF."mc_text_photos_title` where id = '".(int)$id."' ";
      if (!mysql_query($sql))
  ...
  function make_html($id){
    ...
    $sql = "select title, level from `".DB_PREF."mc_text_photos_title` where id = '".(int)$id."' ";
    $rs = mysql_query($sql);
    if ($rs){
      if ($lock = mysql_fetch_assoc($rs)){
        return Template::generateHtml($lock['title'], $lock['level'], $layout);
      }
    }else
  ...
  function manager_preview(){
    ...
    return Template::generateHtml($_REQUEST['title'], $_REQUEST['level'], $_REQUEST['layout']);
  }
