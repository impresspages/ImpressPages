...
class Db{
  ...
  /*public static function getModuleGroup($module_key){
    $sql = "select mg.name from ".DB_PREF."content_module_group mg, ".DB_PREF."content_module m where m.group_id = mg.id and m.name = '".mysql_real_escape_string($module_key)."'";
    $rs = mysql_query($sql);
    if($rs){
