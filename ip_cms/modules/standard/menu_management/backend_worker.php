...
class BackendWorker{
  ...
  public function newPage(){
    ...
    if ($errorCreatedOn || $errorLastModified || $errorEmptyRedirectUrl) {
      ...
      form.property_url.value = \''.addslashes($_POST['url']).'\';
       
       
      var node = iTree.getTree().getSelectionModel().getSelectedNode();
     	node.setText(\''.addslashes($_POST['button_title']).'\');
  ...
  public function getPage(){
    ...
    if (isset($_REQUEST['id']) && isset($_REQUEST['zone_name'])) {
      ...
      form.property_id.value = \''.addslashes($page['id']).'\';
      form.property_button_title.value = \''.addslashes($page['button_title']).'\';
      form.property_page_title.value = \''.addslashes($page['page_title']).'\';
      form.property_keywords.value = \''.addslashes($page['keywords']).'\';
      form.property_description.value = \''.addslashes($page['description']).'\';
      form.property_url.value = \''.addslashes($page['url']).'\';
      form.property_created_on.value = \''.addslashes(substr($page['created_on'], 0, 10)).'\';
      form.property_last_modified.value = \''.addslashes(substr($page['last_modified'], 0, 10)).'\';
      form.property_visible.checked = '.addslashes(($page['visible'])? 'true' : 'false').';
      form.property_rss.checked = '.addslashes(($page['rss'])? 'true' : 'false').';
      document.getElementById(\'property_type_'.$page['type'].'\').checked = true;
      form.property_redirect_url.value = \''.addslashes($page['redirect_url']).'\';
      document.getElementById(\'url_prefix\').innerHTML = \''.addslashes($parentElement->getLink()).'\';      
      document.getElementById(\'url_suffix\').innerHTML = \''.addslashes($page['url']).'\';      
