...
/*  public static function createContentElement($contentId, $languageId){
    global $parametersMod;
    if(Db::contentElement($contentId, $languageId) == null){
      if($parametersMod->getValue('standard', 'menu_management', 'options', 'hide_new_pages'))
        $visible = '0';
      else
        $visible = '1';

      $sql = "insert into ".DB_PREF."content_element set button_title = '', visible= ".$visible.", html=''";
      $rs = mysql_query($sql);      
      if($rs){
        $elementId = mysql_insert_id(); 
        $sql = "insert into ".DB_PREF."zone_to_content set language_id = '".$languageId."', content_id = '".$contentId."', element_id = '".$elementId."' ";
        $rs = mysql_query($sql);
        if($rs){
          return $elementId;
        }else
          trigger_error("Can't bind zone to content ".$sql." ".mysql_error());
      }else{
  ...
  public static function insertContentElement($parent, $rowNumber, $buttonTitle, $pageTitle = '', $keywords = '', $description = '', $url = ''){
    global $parametersMod;
    
    $sql = "update ".DB_PREF."content_element set  row_number = row_number + 1 where parent = '".$parent."' and row_number >= ".$rowNumber." ";
    if (!mysql_query($sql))
    ...
    $sql = "insert into ".DB_PREF."content_element set parent = '".$parent."',row_number = '".$rowNumber."', button_title='".mysql_real_escape_string($buttonTitle)."', page_title='".mysql_real_escape_string($pageTitle)."', keywords='".mysql_real_escape_string($title)."', description='".mysql_real_escape_string($title)."', url='".mysql_real_escape_string(Db::makeUrl($title))."', visible='".$visible."'";
    if (!mysql_query($sql))
      trigger_error("Can't insert new page ".$sql);
    else
