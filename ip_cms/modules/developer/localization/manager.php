...
class Manager{
    ...
    function __construct(){
        ...
        /*$field = new \Library\Php\Form\FieldSelect();
         ...
         $values[] = array('backend', $parametersMod->getValue('Config.language'));
         $values[] = array('frontend', $parametersMod->getValue('Config.language'));
        ...
        if(isset($_GET['action'])){
            switch($_GET['action']){
                ...
                case 'import_uploaded':
                    ...
                    $answer .= HtmlOutput::header();
                    $answer .= '<h1>'.htmlspecialchars($parametersMod->getValue('Config.preview')).'</h1>';
                    $answer .= '<br /><a href="'.$cms->generateUrl($cms->curModId, 'action=import_confirmed').'" class="button">'.htmlspecialchars($parametersMod->getValue('Config.import_language_file')).'</a><br /><br /><br />';
                    $answer .= $this->previewParameters($_SESSION['backend_modules']['developer']['localization']['uploaded_file']);
                    $answer .= '<br /><a href="'.$cms->generateUrl($cms->curModId, 'action=import_confirmed').'" class="button">'.htmlspecialchars($parametersMod->getValue('Config.import_language_file')).'</a><br /><br />';
                    $answer .= HtmlOutput::footer();
                    break;
            ...
            $answer .= '<h1>'.htmlspecialchars($parametersMod->getValue('Config.import_language_file')).'</h1>';
            $answer .= $this->importForm();
            $answer .= '</div><div class="content">';
            $answer .= '<h1>'.htmlspecialchars($parametersMod->getValue('Config.export_language_file')).'</h1>';
