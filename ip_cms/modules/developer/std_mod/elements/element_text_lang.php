...
class ElementTextLang extends Element { //data element in area
    ...
    function __construct($variables) {
        ...
        if(isset($variables['sortable']) && $variables['sortable'] == true) {
            $backtrace = debug_backtrace();
            if(isset($backtrace[0]['file']) && $backtrace[0]['line'])
            trigger_error('ElementTextLang can\'t be sortable. (Error source: '.$backtrace[0]['file'].' line: '.$backtrace[0]['line'].' ) ');
            else
            trigger_error('ElementTextLang can\'t be sortable.');
            exit;
        }
    ...
    function printFieldNew($prefix, $id = null, $area = null) {
        ...
        foreach($languages as $key => $language) {
            ...
            $html->input($prefix.'_'.$language['id'], $this->defaultValues[$language['id']], $this->disabledOnInsert, $this->maxLength);
            else
            $html->input($prefix.'_'.$language['id'], $this->defaultValues[$language['id']], $this->disabledOnInsert, $this->maxLength);
            $html->html("<br />");
        }
    ...
    function printFieldUpdate($prefix, $record, $area) {
        ...
        if (!$rs2) {
            ...
            foreach($languages as $key => $language) {
                ...
                $html->html('<span class="label">'.htmlspecialchars($language['d_short']).'</span> <br />');
                $html->input($prefix.'_'.$language['id'], $value, $this->disabledOnUpdate, $this->maxLength);
                $html->html("<br />");
            }
