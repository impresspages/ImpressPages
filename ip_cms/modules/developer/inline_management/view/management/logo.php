...
<?php if (isset($type) && $type == 'image') { ?>
    <a href="<?php echo isset($link) ? $link : '' ?>" style="<?php echo !empty($color) ? 'color: '.htmlspecialchars($color).';' : '' ?> <?php echo !empty($font) ? 'font-family: '.htmlspecialchars($font).';' : '' ?>">
        <img <?php echo $empty ? 'style="width: 50px; height: 50px;"' : '' ?> src="<?php echo BASE_URL.($this->esc(!$empty ? $image : MODULE_DIR.'developer/inline_management/public/empty.gif')) ?>" alt="<?php echo $this->escPar('standard/configuration/main_parameters/name'); ?>" />
    </a>
<?php } else { ?>
    <a href="<?php echo isset($link) ? $link : '' ?>" style="<?php echo !empty($color) ? 'color: '.htmlspecialchars($color).';' : '' ?> <?php echo !empty($font) ? 'font-family: '.htmlspecialchars($font).';' : '' ?>">
