...
<input '.$this->getAttributesStr($doctype).' style="display: none;" class="'.implode(' ',$this->getClasses()).'" name="'.htmlspecialchars($this->getName()).'[]"  '.$this->getValidationAttributesStr($doctype).' type="hidden" value="'.htmlspecialchars(md5(date("Y-m-d"). \Ip\Config::getRaw('SESSION_NAME'))).'" />
';
    }
