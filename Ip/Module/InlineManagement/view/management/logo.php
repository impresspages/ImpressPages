...
<?php if (isset($type) && $type == 'image') { ?>
    <a href="<?php echo isset($link) ? $link : '' ?>" style="<?php echo !empty($color) ? 'color: '.htmlspecialchars($color).';' : '' ?> <?php echo !empty($font) ? 'font-family: '.htmlspecialchars($font).';' : '' ?>">
        <img <?php echo $empty ? 'style="width: 50px; height: 50px;"' : '' ?> src="<?php echo (esc(!$empty ? ipConfig()->baseUrl($image) : ipConfig()->coreModuleUrl('InlineManagement/assets/empty.gif'))) ?>" alt="<?php echo esc(ipGetOption('Config.websiteTitle')) ?>" />
    </a>
<?php } else { ?>
    <a href="<?php echo isset($link) ? $link : '' ?>" style="<?php echo !empty($color) ? 'color: '.htmlspecialchars($color).';' : '' ?> <?php echo !empty($font) ? 'font-family: '.htmlspecialchars($font).';' : '' ?>">
