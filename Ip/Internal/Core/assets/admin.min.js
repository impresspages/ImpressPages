...
if(typeof ipBrowseLinkModalTemplate!=="undefined"){$('body').append(ipBrowseLinkModalTemplate);}};this.setManagementMode=function(mode){$.ajax({url:ip.baseUrl,dataType:'json',type:'POST',data:{aa:'Content.setManagementMode',value:mode,securityToken:ip.securityToken},success:function(response){if(response){if(getParameterByName('_revision')){window.location=window.location.href.split('#')[0].split('?')[0]+'?_revision='+ip.revisionId;}else{window.location=window.location.href.split('#')[0].split('?')[0]}}else{window.location=ip.baseUrl+'admin';}},error:function(response){alert('error: '+response);}});};var getParameterByName=function(name){name=name.replace(/[\[]/,"\\[").replace(/[\]]/,"\\]");var regex=new RegExp("[\\?&]"+name+"=([^&#]*)"),results=regex.exec(location.search);return results==null?"":decodeURIComponent(results[1].replace(/\+/g," "));}};function ipBrowseLink(callback){var selectedPageId=null;var $modal=$('#ipBrowseLinkModal'),$iframe=$modal.find('.ipsPageSelectIframe');$iframe.attr('src',$iframe.data('source'));$modal.modal();var $iframeContent=$iframe.contents();$modal.find('.ipsConfirm').on('click',function(){var iframeWindow=$iframe.get(0).contentWindow;selectedPageId=iframeWindow.angular.element(iframeWindow.$('.ipAdminPages')).scope().selectedPageId;$modal.modal('hide');});$modal.off('hide.bs.modal').on('hide.bs.modal',function(){if(!selectedPageId){callback('');return;}
