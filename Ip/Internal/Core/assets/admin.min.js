...
if(typeof ipBrowseLinkModalTemplate!=="undefined"){$('body').append(ipBrowseLinkModalTemplate);}};this.setManagementMode=function(mode){$.ajax({url:ip.baseUrl,dataType:'json',type:'POST',data:{aa:'Content.setManagementMode',value:mode,securityToken:ip.securityToken},success:function(response){if(response){window.location=window.location.href.split('#')[0];}else{window.location=ip.baseUrl+'admin';}},error:function(response){alert('error: '+response);}});};};})(ip.jQuery);function ipGetPageTree()
{var $=ip.jQuery;var data={};data.aa='Content.getPageTree';data.securityToken=ip.securityToken;return $.ajax({type:'GET',url:ip.baseUrl,data:data,dataType:'json'});}
function ipBrowseLink(callback_function){var $=ip.jQuery,$modal=$('#ipBrowseLinkModal'),$tree=$('.ipSitemap',$modal);ipGetPageTree().success(function(data){$tree.jstree({core:{data:data.sitemap,animation:0,multiple:false,themes:{dots:false}},plugins:['wholerow']});$tree.on('select_node.jstree',function(){$('.btn-primary',$modal).removeClass('disabled');});$modal.modal('show').on('click','.btn-primary',function(){var url=$('#'+$tree.jstree('get_selected')[0]).data('url');$modal.modal('hide');$tree.jstree('destroy');if(callback_function){callback_function(url);}else{console.log(url);}});});}
(function(jQuery){
/*!
