...
if(!e.isDefaultPrevented()){$.ajax({url:$form.attr('action')?$form.attr('action'):ip.baseUrl,dataType:'json',type:type,data:$form.serialize(),success:function(response){$form.trigger('ipSubmitResponse',[response]);if(response.errors){$form.data("validator").invalidate(response.errors);}
...
$.tools.validator.localize(key,value);});})(jQuery);jQuery('.ipWidget-Form').on('ipSubmitResponse',function(e,response){function isScrolledIntoView(elem)
{var docViewTop=$(window).scrollTop();var docViewBottom=docViewTop+$(window).height();var elemTop=$(elem).offset().top;var elemBottom=elemTop+$(elem).height();return((elemBottom<=docViewBottom)&&(elemTop>=docViewTop));}
var $widget=$(this);if(response.status&&response.status=='success'){var $success=$widget.find('.ipwSuccess');$success.show();$widget.find('.ipwForm').hide();if(!isScrolledIntoView($widget.find('.ipwSuccess'))){$('html, body').animate({scrollTop:$success.offset().top},500);}}});jQuery.fn.ipWidgetMap=function(){"use strict";return this.each(function(){if(ip.isManagementState){return;}
