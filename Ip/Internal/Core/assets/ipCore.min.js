...
$('.ipsModuleFormPublic .ipsFileContainer').ipFormFile();$('.ipsModuleFormPublic .type-richtext').ipFormRichtext();$('.ipsModuleFormPublic .type-url').ipFormUrl();$('.ipsModuleFormPublic .type-color').ipFormColor();$('.ipsModuleFormPublic').each(function(){var $form=$(this);if($form.find(":submit").length==0){$form.append('<input type="submit" style="position: absolute; left: -999999px; width: 1px; height: 1px; visibility: hidden;" tabindex="-1" />');}});};};})(jQuery);var validatorConfig='';var ipValidator;(function($){"use strict";ipValidator=new function(){this.init=function(){$.each(ipValidatorTranslations,function(key,value){if(validatorConfig===''){validatorConfig=createConfig(key);}
$.tools.validator.localize(key,value);});};var createConfig=function(translationsKey){var config={'lang':ip.languageUrl,'messageClass':'hidden','onFail':function(e,errors){$.each(errors,function(){var err=this;var $control=this.input;$control.parents('.form-group').addClass('has-error').find('.help-error').html(this.messages.join(' '));if(this.messages.join('')==''){$control.parents('.form-group').find('.help-error').hide()}else{$control.parents('.form-group').find('.help-error').show()}});},'onSuccess':function(e,valids){$.each(valids,function(){var $control=$(this);$control.parents('.form-group').removeClass('has-error');});}};return config;};};ipValidator.init();})(jQuery);jQuery.fn.ipWidgetForm=function(){"use strict";return this.each(function(){var $ipForm=$(this);$ipForm.find('form').validator(validatorConfig);$ipForm.find('form').submit(function(e){var form=$(this);if(!e.isDefaultPrevented()){$.ajax({url:ip.baseUrl,dataType:'json',type:'POST',data:form.serialize(),success:function(response){if(response.status&&response.status=='success'){if(typeof ipWidgetFormSuccess=='function'){ipWidgetFormSuccess($ipForm);}else{$ipForm.find('.ipwSuccess').show();$ipForm.find('.ipwForm').hide();}}else{if(response.errors){form.data("validator").invalidate(response.errors);}}}});}
e.preventDefault();});});};jQuery.fn.ipWidgetMap=function(){"use strict";return this.each(function(){if(ip.isManagementState){return;}
...
var map=new google.maps.Map($map.get(0),mapOptions);if((typeof($map.data('markerlat')!=='undefined'))&&(typeof($map.data('markerlng')!=='undefined'))){var marker=new google.maps.Marker({position:new google.maps.LatLng($map.data('markerlat'),$map.data('markerlng')),map:map});}}});};var ipGoogleMapsLoading=false;$(document).ready(function(){$('.ipWidget-Form').ipWidgetForm();if($('.ipWidget-Map').length){$('body').on('ipGoogleMapsLoaded',function(){$('.ipWidget-Map').ipWidgetMap();});if(typeof(google)!=='undefined'&&typeof(google.maps)!=='undefined'&&typeof(google.maps.LatLng)!=='undefined'){ipGoogleMapsLoaded();}else{ipLoadGoogleMaps();}}});var ipGoogleMapsLoaded=function(){$('body').trigger('ipGoogleMapsLoaded');}
var ipLoadGoogleMaps=function(){if(ipGoogleMapsLoading){return;}
ipGoogleMapsLoading=true;var script=document.createElement('script');script.type='text/javascript';script.src='https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&libraries=places&'+'callback=ipGoogleMapsLoaded';document.body.appendChild(script);}
var ipInitForms=function(){ipModuleFormAdmin.init();ipModuleFormPublic.init();}