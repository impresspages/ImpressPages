...
function ipHomeUrl()
{return ip.homeUrl;}
function ipInitForms(){if(ipModuleFormAdmin){ipModuleFormAdmin.init();}
if(ipModuleFormPublic){ipModuleFormPublic.init();}}
(function(jQuery){
/*!
...
$('.ipsModuleFormPublic .ipsFileContainer').ipFormFile();$('.ipsModuleFormPublic .type-richtext').ipFormRichtext();$('.ipsModuleFormPublic .type-color').ipFormColor();$('.ipsModuleFormPublic').each(function(){var $form=$(this);if($form.find(":submit").length==0){$form.append('<input type="submit" style="position: absolute; left: -999999px; width: 1px; height: 1px; visibility: hidden;" tabindex="-1" />');}});};};})(jQuery);var validatorConfigPublic='';(function($){"use strict";var createConfig=function(translationsKey){var config={'lang':ip.languageUrl,'messageClass':'hidden','onFail':function(e,errors){$.each(errors,function(){var err=this;var $control=this.input;$control.parents('.form-group').addClass('has-error').find('.help-error').html(this.messages.join(' '));if(this.messages.join('')==''){$control.parents('.form-group').find('.help-error').hide()}else{$control.parents('.form-group').find('.help-error').show()}});},'onSuccess':function(e,valids){$.each(valids,function(){var $control=$(this);$control.parents('.form-group').removeClass('has-error');});}};return config;};$.each(ipValidatorTranslations,function(key,value){if(validatorConfigPublic===''){validatorConfigPublic=createConfig(key);}
$.tools.validator.localize(key,value);});$('.ipsModuleFormPublic.ipsAjaxSubmit').validator(validatorConfigPublic);$('.ipsModuleFormPublic.ipsAjaxSubmit').submit(function(e){var $form=$(this);if(!e.isDefaultPrevented()){$.ajax({url:ip.baseUrl,dataType:'json',type:'POST',data:$form.serialize(),success:function(response){$form.trigger('ipSubmitResponse',[response]);if(response.errors){$form.data("validator").invalidate(response.errors);}
...
e.preventDefault();});})(jQuery);jQuery('.ipWidget-Form').on('ipSubmitResponse',function(e,response){var $widget=$(this);if(response.status&&response.status=='success'){$widget.find('.ipwSuccess').show();$widget.find('.ipwForm').hide();}});jQuery.fn.ipWidgetMap=function(){"use strict";return this.each(function(){if(ip.isManagementState){return;}
