...
var $widgetControls=$this.find(' > .ip > .ipsWidgetControls');$widgetControls.find('.ipsWidgetDelete').on('click',function(e){e.preventDefault();ipContent.deleteWidget($this.data('widgetinstanceid'));});$widgetControls.find('.ipsContainer').on('shown.bs.dropdown',function(){$widgetControls.css('z-index',($widgetControls.zIndex()+1));}).on('hidden.bs.dropdown',function(){$widgetControls.css('z-index','');});$widgetControls.find('.ipsSkin').on('click',$.proxy(openLayoutModal,this));});},widgetController:function(){return this.data('widgetController');},save:function(widgetData,refresh,callback){return this.each(function(){var $this=$(this);var $queue=$this.data('saveQueue');if($queue==null){$queue=[];}
