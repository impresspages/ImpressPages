...
options.fieldTemplate=this.modal.find('.ipsFieldTemplate');options.optionsPopup=$("#ipWidgetFormPopupOptions").ipWidget_ipForm_options({fieldTypes:instanceData.fieldTypes});this.container.ipWidget_ipForm_container(options);this.addButton.on('click',$.proxy(addField,this));var customTinyMceConfig=ipTinyMceConfig();this.modal.find('.ipsSuccess').html(instanceData.success);this.modal.find('.ipsSuccess').tinymce(customTinyMceConfig);if(instanceData.sendTo=='custom'){this.modal.find('input[value=custom]').prop('checked',true);}else{this.modal.find('input[value=default]').prop('checked',true);}
...
this.filesSelected=function(event,files){var $this=$(this);var data=this.data;var data={method:'update'};$.each(files,function(key,value){data.newImage=value.fileName;});this.$widgetObject.save(data,1);}
var updateImage=function(newImage){var $this=$(this);var data=this.data;var data={method:'add'};this.$widgetObject.save(data,1,function($widget){$widget.click();});}
