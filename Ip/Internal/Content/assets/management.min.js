...
this.focusImage=function(e){console.log('focus');var context=this;e.preventDefault();var $item=$(e.currentTarget);var $img=$item.find('.ipsImage');var $controls=this.$controls;console.log($controls);$controls.removeClass('hide');$controls.css('position','absolute');$controls.css('left',$img.offset().left+5);$controls.css('top',$img.offset().top+5);this.imageIndex=$item.index();$controls.find('.ipsDelete').off().on('click',function(e){$.proxy(context.deleteImage,context)($item.index());});$controls.find('.ipsEdit').off().on('click',function(e){$.proxy(context.editImage,context)($item.index());})
$controls.find('.ipsLink').off().on('click',function(e){$.proxy(linkPopup,context)($item.index());});};this.blurImage=function(){console.log('hide1')
...
this.blur=function(){console.log('hide2')
var $widgetControls=this.$widgetControls;$widgetControls.addClass('hide');this.$controls.addClass('hide');};this.deleteImage=function(position){if(!this.data.images[1]){ipContent.deleteWidget(this.$widgetObject.data('widgetinstanceid'));return;}
...
this.focusImage=function(e){var context=this;e.preventDefault();var $item=$(e.currentTarget);var $img=$item.find('.ipsImage');var $controls=this.$controls;$controls.removeClass('hide');$controls.css('position','absolute');$controls.css('left',$img.offset().left+5);$controls.css('top',$img.offset().top+5);$controls.find('.ipsDelete').off().on('click',function(e){$.proxy(context.deleteImage,context)($item.index());});$controls.find('.ipsEdit').off().on('click',function(e){$.proxy(context.editImage,context)($item.index());});$controls.find('.ipsLink').off().on('click',function(e){$.proxy(linkPopup,context)($item.index());});};this.blurImage=function(){this.$controls.addClass('hide');};this.editImage=function(position){var thisContext=this;var $modal=$('#ipWidgetImageEditPopup');var options=new Object;var data=this.data;$modal.modal();if(data.imageOriginal){options.image=data.imageOriginal;}
if(data.cropX1){options.cropX1=data.cropX1;}
if(data.cropY1){options.cropY1=data.cropY1;}
if(data.cropX2){options.cropX2=data.cropX2;}
if(data.cropY2){options.cropY2=data.cropY2;}
...
var linkPopup=function(index){var context=this;this.popup=$('#ipWidgetImageLinkPopup');this.confirmButton=this.popup.find('.ipsConfirm');this.type=this.popup.find('select[name=type]');this.url=this.popup.find('input[name=url]');this.blank=this.popup.find('input[name=blank]');var data=this.data;if(data.type){this.type.val(data.type);}else{this.type.val('lightbox');}
if(data.url){this.url.val(data.url);}else{this.url.val('');}
if(data.blank){this.blank.attr('checked',true);}else{this.blank.attr('checked',false);}
this.type.off().on('change',function(){$.proxy(showHide,context)();});$.proxy(showHide,context)();this.popup.modal();this.confirmButton.off().on('click',$.proxy(saveLink,this));};var saveLink=function(){var data={method:'setLink',type:this.type.val(),url:this.url.val(),blank:this.blank.attr('checked')?1:0};this.$widgetObject.save(data,1);this.popup.modal('hide');};var showHide=function(){if(this.type.val()=='link'){this.popup.find('.form-group.name-url').show();this.popup.find('.form-group.name-blank').show();}else{this.popup.find('.form-group.name-url').hide();this.popup.find('.form-group.name-blank').hide();}}};})(ip.jQuery);var IpWidget_Text;(function($){"use strict";IpWidget_Text=function(){this.$widgetObject=null;this.init=function($widgetObject,data){var customTinyMceConfig=ipTinyMceConfig();this.$widgetObject=$widgetObject;customTinyMceConfig.setup=function(ed,l){ed.on('change',function(e){$widgetObject.save({text:$widgetObject.find('.ipsContent').html()});})};$widgetObject.find('.ipsContent').tinymce(customTinyMceConfig);};this.onAdd=function(){this.$widgetObject.find('.ipsContent').focus();}
...
this.$widgetObject.on('remove',$.proxy(this.destroy,this));if(!this.$header.css('min-height')||this.$header.css('min-height')=='0px'){this.$header.css('min-height',this.$header.css('font-size'));}};this.onAdd=function(){if(this.$widgetObject.index()!=0){this.data.level=2;this.save(true);}else{var $headTag=this.$widgetObject.find('h1,h2,h3,h4,h5,h6');$headTag.focus();}}
