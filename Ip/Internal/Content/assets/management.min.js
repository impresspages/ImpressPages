...
var $columnsWidget=$block.closest('.ipWidget-Columns');if($columnsWidget.length){deleteEmptyColumns($columnsWidget.data('widgetinstanceid'),function(){if(callback){callback();}});}else{if(callback){callback();}}},dataType:'json'});}
...
$(document).ipContentManagement('initBlocks',$newWidget.find('.ipBlock'));$block.trigger('reinitRequired.ipWidget');$block.trigger('addWidget.ipWidget',{'instanceId':response.instanceId,'widget':$newWidget});var widgetController=$newWidget.ipWidget('widgetController');if(widgetController&&typeof(widgetController['onAdd'])==='function'){widgetController.onAdd($newWidget);}}
if(callback){callback($newWidget.data('widgetinstanceid'));}},dataType:'json'});};this.moveWidget=function(instanceId,position,block,callback){var $originalBlock=$('#ipWidget-'+instanceId).closest('.ipBlock');moveWidgetHelper(instanceId,position,block,function(newInstanceId){var $columns=$originalBlock.closest('.ipWidget-Columns');if($columns.length){deleteEmptyColumns($columns.data('widgetinstanceid'),function(){if(callback){callback(response.newInstanceId);}
return;});}
if(callback){callback(newInstanceId);}});};var moveWidgetHelper=function(instanceId,position,block,callback){var data=Object();data.aa='Content.moveWidget';data.securityToken=ip.securityToken;data.instanceId=instanceId;data.position=position;data.blockName=block;var $block=$('#ipBlock-'+block);data.revisionId=$block.data('revisionid');data.languageId=$block.data('languageid');var $originalBlock=$('#ipWidget-'+instanceId).closest('.ipBlock');$.ajax({type:'POST',url:ip.baseUrl,data:data,success:function(response){if(response.status=='error'){alert(response.errorMessage);}
...
touchHandled=false;};mouseProto._mouseInit=function(){var self=this;self.element.bind('touchstart',$.proxy(self,'_touchStart')).bind('touchmove',$.proxy(self,'_touchMove')).bind('touchend',$.proxy(self,'_touchEnd'));_mouseInit.call(self);};})(ip.jQuery);(function($){"use strict";$(document).bind('initFinished.ipContentManagement',function(){$('.ipsAdminPanelWidgetButton').bind('dragstart',function(event,ui){$('.ipBlock > .ipbExampleContent').each(function(){var $ipExampleContent=$(this);var $block=$ipExampleContent.parent();if($block.css('min-height')){$block.data('ipMinHeight',$block.css('min-height'));}
...
$.ajax({type:'POST',url:ip.baseUrl,data:postData,context:$this,success:methods._savePageResponse,dataType:'json'});});},_savePageResponse:function(response){var $this=$(this);var data=$this.data('ipContentManagement');if(response.status=='success'){window.location.href=response.newRevisionUrl;}else{}}};var ipAdminWidgetsScroll=function(){var $scrollable=$('.ipsAdminPanelWidgetsContainer');$scrollable.scrollable({items:'li',touch:false});var scrollableAPI=$scrollable.data('scrollable');var itemWidth=scrollableAPI.getItems().eq(0).outerWidth(true);var containerWidth=scrollableAPI.getRoot().width()+24;var scrollBy=Math.floor(containerWidth/itemWidth);if(scrollBy<1){scrollBy=1;}
...
options.fieldTemplate=this.modal.find('.ipsFieldTemplate');options.optionsPopup=$("#ipWidgetFormPopupOptions").ipWidget_ipForm_options({fieldTypes:instanceData.fieldTypes});this.container.ipWidget_ipForm_container('destroy');this.container.ipWidget_ipForm_container(options);this.addButton.on('click',$.proxy(addField,this));this.modal.find('textarea[name=success]').val(instanceData.success);if(instanceData.sendTo=='custom'){this.modal.find('select[name=sendTo]').val('custom');this.modal.find('.form-group.name-emails').show();}else{this.modal.find('select[name=sendTo]').val('default');this.modal.find('.form-group.name-emails').hide();}
...
this.modal.find('input[name=emails]').val(instanceData.emails);this.modal.find('.ipsTabs li a').first().click();IpWidget_Form.init();};var cleanup=function(){this.container.html('');this.container.ipWidget_ipForm_container('destroy');this.addButton.off();this.confirmButton.off();}
...
var map=new google.maps.Map($map.get(0),mapOptions);this.map=map;if((typeof(data.markerlat)!=='undefined')&&(typeof(data.markerlng)!=='undefined')){this.marker=new google.maps.Marker({position:new google.maps.LatLng(data.markerlat,data.markerlng),map:this.map});}
