...
class AdminController extends \Ip\Controller
{
    ...
    public function index()
    {
        ...
        $data = array (
            ...
            'imageDir' => ipFile('Ip/Internal/Pages/img/')
        );
            ...
            case 'zone' : //parent node is zone
                ...
                foreach ($items as $itemKey => $item) {
                    ...
                    if ($item['visible']) {
                        ...
                        $icon = ipFileUrl('Ip/Internal/Pages/img/file_hidden.png');
                    }
    ...
    public function createPage () {
        ...
        if (isset($_REQUEST['languageId'])) {
            $language = \Ip\ServiceLocator::content()->getLanguage($_REQUEST['languageId']);
        } else {
            $languages = Db::getLanguages();
            $languageArray = $languages[0];
            $language = \Ip\ServiceLocator::content()->getLanguage($languageArray['id']);
        }
        ...
        $element = \Ip\Internal\Content\DbFrontend::getPage($tmpId);
        while($element['parent'] !== null) {
            $tmpUrlVars[] = $element['url'];
            $element = \Ip\Internal\Content\DbFrontend::getPage($element['parent']);
        }
        $languageId = \Ip\Internal\Content\DbFrontend::languageByRootPage($element['id']);
        //end find language
