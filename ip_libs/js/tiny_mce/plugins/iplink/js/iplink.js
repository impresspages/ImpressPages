...
//  LibDefault.ajaxMessage(parent.global_config_modules_url+'standard/content_management/sitemap_list.php', 'action=sitemap_list&current_href=' + encodeURIComponent(href), updateIPlinks)
  LibDefault.ajaxMessage(parent.window.location, 'action=sitemap_list&current_href=' + encodeURIComponent(href), updateIPlinks)
}
...
function sitemapFunctions(objID) {
	...
	if(sitemap){
		...
		for(var i=0;i<anchors.length;i++){
			anchors[i].onclick = function(){ document.getElementById('href').value = this.href; return false; }
		}
